// Get ethers from setupTestSystem
import { ethers, getHardhatEthers } from "./setupTestSystem.js";

// Import ethers from the package directly
import { ethers as standaloneEthers } from "ethers";

// Define BigNumberish type locally
...
export function fromBN(val: BigNumberish, dec?: number): string {
  const hardhatEthersInstance = getHardhatEthers();
  if (!hardhatEthersInstance) {
    return standaloneEthers.formatUnits(val, dec || 18);
  }
  return hardhatEthersInstance.formatUnits(val, dec || 18);
}
