...
async function deployProdSystem(): Promise<ProdSystemContracts> {
  ...
  let {
    ...
    repositoryAddress,
    accountNFTAddress,
  } = loadContractAddresses("CmeCoveredCall", chainId);
  ...
  const deployedRepositories = await repositoryFactory.deployedRepositories(1);
  ...
  if (accountNFTAddress && accountNFTAddress != "") {
    await bookKeeper.setAccountNFT(accountNFTAddress, 3);
  } else {
...
deployProdSystem().then((prodSystem: any) =>
  dumpDeploymentsToFile(prodSystem, "prodSystemCMECC", "deployments")
    .then(() => verifyContractsFromFile("./prodSystemCMECC.json"))
