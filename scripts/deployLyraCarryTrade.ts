...
async function deployProdSystem(): Promise<ProdSystemContracts> {
  ...
  let WETH, repositoryFactory, bookKeeper, gateKeeper;
  let wethAddress = addressConfig[networkName].wethAddress || "";
  ...
  let {
    ...
    gateKeeperAddress,
    repositoryAddress,
  } = loadContractAddresses("LyraCarryTrade", chainId);

  // WETH
  if (wethAddress && wethAddress != "") {
    console.log(chalk.green(`used existing WETH: ${wethAddress}`));
    ...
    WETH = (await (
      await ethers.deployContract("TestERC20SetDecimals", ["WTH", "WETH", 6])
    ).waitForDeployment()) as unknown as TestERC20SetDecimals;
    console.log(chalk.green(`deployed mock WETH: ${await WETH.getAddress()}`));
  }
...
deployProdSystem().then((prodSystem: any) =>
  dumpDeploymentsToFile(prodSystem, "prodSystemLyraCT", "deployments")
    .then(() => verifyContractsFromFile("./prodSystemLyraCT.json"))
